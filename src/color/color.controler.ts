...
export function normalizeColorInput(
  ...
  Object.keys(req.body.normalizeColorInput).forEach((key) => {
    if (req.body.normalizeColorInput[key] === undefined)
      delete req.body.normalizeColorInput[key];
  });
  next();
}
...
export async function add(req: Request, res: Response) {
  try {
    ...
    res
      .status(201)
      .json({ message: 'Color successfully created.', data: color });
  } catch (error: any) {
...
export async function update(req: Request, res: Response) {
  try {
    ...
    res
      .status(201)
      .json({ message: 'Color successfully updated.', data: colorToUpdate });
  } catch (error: any) {
    if (error instanceof ZodError) {
      const { fieldErrors: errors } = error.flatten();
      res.status(500).json({ message: errors });
...
export async function remove(req: Request, res: Response) {
  try {
    ...
    try {
      ...
      res
        .status(200)
        .json({ message: `Color with id=${id} successfully deleted.` });
    } catch (error: any) {
      if (
