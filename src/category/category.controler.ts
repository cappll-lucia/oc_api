...
export function normalizeCategoryInput(
  ...
  Object.keys(req.body.normalizeCategoryInput).forEach((key) => {
    if (req.body.normalizeCategoryInput[key] === undefined)
      delete req.body.normalizeCategoryInput[key];
  });
  next();
}
...
export async function add(req: Request, res: Response) {
  try {
    ...
    res
      .status(201)
      .json({ message: 'Category successfully created', data: category });
  } catch (error: any) {
